...
function calcPP() {
    ...
    let strainValue = Math.pow(5 * Math.max(1.0, strain / 0.0075) - 4.0, 2.0) / 100000.0;
    ...
    strainValue *= Math.pow(0.99, misses);

    let accValue = Math.pow(150 / OD300, 1.1) * Math.pow(acc / 100, 15) * 21.0;
    accValue *= Math.min(Math.pow(hitcount / 1500, 0.3), 1.15);

    let globalMultiplier = 1.1;

    if (document.getElementById("HD").checked) {
        globalMultiplier *= 1.125
        strainValue *= 1.025
    }
